...
StackResult stackPush(Stack* stack, char bracket, int position){
    ...
    if(stack->top + 1 == stack->capacity){
        if (stack->capacity==stack->max_capacity){
            ...
            StackElem*temp=realloc(stack->data,((stack->capacity)+1)*sizeof(char));
            if(!temp){
                return STACK_MEM_ERR;
            }
            stack->data = temp;
...
int jsonBracketCheckBasic(const char *json_str) {
    ...
    Stack stack;
    StackElem elem;
    stackInit(&stack, INIT_capacity, MAX_capacity);
    // 步骤3：遍历json_str的每个字符：
    //        - 遇到左括号（{/[）：压栈（stackPush）
...
// 步骤5：遍历结束后，检查栈是否为空且不在字符串内，两者都满足则返回1，否则返回0
        if(stackIsEmpty(&stack) && in_string == 0){
            stackDestroy(&stack);
            return 1;
        }else{
            stackDestroy(&stack);
